...
const Deckshow = () => {
    ...
    const showNextCard = () => {
        setCurrentCardIndex((prevIndex) => Math.min(prevIndex + 1, deck.cards.length - 1));
    };
    ...
    return (
        <div>
            ...
            {studyMode ? (
                <>
                    <Card key={deck.cards[currentCardIndex]._id} card={deck.cards[currentCardIndex]} />
            ...
            <button onClick={toggleStudyMode}>{studyMode ? "Exit Study Mode" : "Study Mode"}</button>
        </div>
    );
};
